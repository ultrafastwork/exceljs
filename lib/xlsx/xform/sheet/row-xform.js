...
class RowXform extends BaseXform {
  ...
  parseOpen(node) {
    ...
    if (node.name === 'row') {
      const rowNumber = node.attributes.r ? parseInt(node.attributes.r, 10) : (this.currentRowNumber || 0) + 1;
      this.currentRowNumber = rowNumber;
      this.currentColumnNumber = 0; // reset column counter at the start of each row
    ...
    if (this.parser) {
      if (node.name === 'c') {
        ...
        if (node.attributes.r) {
          ...
          colNumber = (this.currentColumnNumber || 0) + 1;
          node.attributes.r = colCache.encodeAddress(this.currentRowNumber, colNumber);
        }
        this.currentColumnNumber = colNumber;
      }
      this.parser.parseOpen(node);
      return true;
    }
