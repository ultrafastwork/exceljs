...
class Worksheet {
  ...
  _mergeCellsInternal(dimensions, ignoreStyle) {
    // 1. Perform atomic check first to prevent partial merge state corruption
    // and use findCell to avoid instantiating empty cells.
    ...
    // 2. Apply merge now that range is verified safe
  ...
  removeImage(sheetImageId) {
    this._media = this._media.filter(m => m.sheetImageId !== sheetImageId);
  }
...
Please leave feedback at https://github.com/exceljs/exceljs/discussions/2575`
  ...
  set model(value) {
    ...
    this._media = value.media.map(medium => {
      const sheetImageId = uuidv4();
    ...
    this.tables = value.tables.reduce((tables, table) => {
      ...
      // Add empty rows array if missing, and populate from worksheet cells if they exist
      if (!table.rows || table.rows.length === 0) {
        table.rows = [];
        if (table.ref) {
          const range = colCache.decode(table.ref);
          const startRow = range.top + (table.headerRow ? 1 : 0);
          const endRow = range.bottom - (table.totalsRow ? 1 : 0);
