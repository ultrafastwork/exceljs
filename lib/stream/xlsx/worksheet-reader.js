        ...
        if (eventType === 'opentag') {
          ...
          if (emitSheet) {
            switch (node.name) {
              ...
              case 'row':
                if (inRows) {
                  const r = node.attributes.r ? parseInt(node.attributes.r, 10) : (currentRowNumber || 0) + 1;
                  currentRowNumber = r;
              ...
              case 'c':
                if (row) {
                  let ref = node.attributes.r;
                  let colNumber;
                  if (ref) {
                    const address = colCache.decodeAddress(ref);
                    colNumber = address.col;
                  } else {
                    colNumber = (currentColumnNumber || 0) + 1;
                    ref = colCache.encodeAddress(currentRowNumber, colNumber);
                  }
          ...
          if (emitSheet) {
            switch (node.name) {
              ...
              case 'mergeCell':
                if (mergeCell) {
                  worksheetEvents.push({eventType: 'mergeCell', value: mergeCell});
                }
                mergeCell = null;
                break;
              default:
